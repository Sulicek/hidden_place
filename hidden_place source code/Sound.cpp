	...
	float loopStartSeconds,float loopEndSeconds )
{
	...
	try
	{
		...
		// compare format with sound system format
		{
			...
			{
				std::string msg = "bad wave format (nChannels)" + std::to_string(format.nChannels) + " != " + std::to_string(sysFormat.nChannels);
				std::wstring msgw(msg.begin(), msg.end());
				throw CHILI_SOUND_FILE_EXCEPTION( fileName,msgw);
			}
